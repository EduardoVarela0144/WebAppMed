...
<div id="createProductModal" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative p-4 w-full max-w-3xl h-full md:h-auto">
        ...
        <div class="relative p-4 bg-white rounded-lg shadow  sm:p-5">
            ...
            <form:form action="/saveSchedule?page=schedules" method="post" modelAttribute="schedule">
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <form:select type="text" path="id_doctor" id="id_doctor"
                                     class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-black focus:border-black block w-full p-2.5">
                            <option selected="" value="">Selecciona al doctor</option>
                            <c:forEach var="doctor" items="${listUsers}">
                                <c:if test="${not empty doctor.id_doctor}">
                                <option required="true" value="${doctor.id_user}">Dr. ${doctor.name} ${doctor.ape_pat} ${doctor.ape_mat}</option>
                                </c:if>
                            </c:forEach>
                        </form:select>
                    </div>
